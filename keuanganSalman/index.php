...
$pengeluaran_hari_ini_query = mysqli_query($koneksi, "SELECT SUM(jumlah) as total_pengeluaran FROM pengeluaran WHERE tgl_pengeluaran = CURDATE()");
$pengeluaran_hari_ini = mysqli_fetch_array($pengeluaran_hari_ini_query);
$total_pengeluaran = $pengeluaran_hari_ini['total_pengeluaran'];
if ($pengeluaran_hari_ini !== null) {
    $jumlah_pengeluaran_hari_ini = $pengeluaran_hari_ini['jumlah'];
} else {
    $jumlah_pengeluaran_hari_ini = 0;
}
...
$jumlahmasuk = count($arraymasuk) > 0 ? array_sum($arraymasuk) : 0;
...
$jumlahkeluar = count($arraykeluar) > 0 ? array_sum($arraykeluar) : 0;
...
$pemasukan_hari_ini_query = mysqli_query($koneksi, "SELECT SUM(jumlah) as total_pemasukan FROM pemasukan WHERE tgl_pemasukan = CURDATE()");
$pemasukan_hari_ini = mysqli_fetch_array($pemasukan_hari_ini_query);
$total_pemasukan = $pemasukan_hari_ini['total_pemasukan'];

if ($pemasukan_hari_ini !== null) {
    $jumlah_pemasukan_hari_ini = $pemasukan_hari_ini['jumlah'];
} else {
    $jumlah_pemasukan_hari_ini = 0;
}
                ...
                <div class="card-body">
                  <div class="row no-gutters align-items-center">
                    <div class="col mr-2">
                      ...
                      <div class="h5 mb-0 font-weight-bold text-gray-800">Rp.<?=number_format($total_pemasukan, 2, ',', '.');?></div>
      ...
      tooltip: {
        ...
        callbacks: {
          label: function(tooltipItem) {
            return 'Pendapatan: Rp.' + tooltipItem.raw;
          }
        }
      }
    }
